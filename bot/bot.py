    ...
    if message.content.startswith("!전투력"):
        ...
        if message.content[5:].startswith("측정"):
            if message.content[12:].startswith("도움"):
                ...
                global con11, con12, con13, con14, con15, con21, con22, con23, con24, con25, result, nj, boost1, boost2
                nj = 0
                if con1 == con2:
                ...
                if int(message.content[68:71]) > 0 and int(message.content[72:75]) > 0:
                    boost1 = int(message.content[68:71])
                    boost2 = int(message.content[72:75])
                elif int(message.content[68:71]) <= 0 and int(message.content[72:75]) <= 0:
                    await message.channel.send("멍청아. 전략/전술 부스트는 음수거나 0일 수 없어")
                else:
                    boost1 = 1
                    boost2 = 1
                if message.content[8:].startswith("FLT"):
                    conr1 = con11*3 + con12*4 + con13 + con14 + con15*2.5 * boost1
                    conr2 = con21*3 + con22*4 + con23 + con24 + con25*2.5 * boost2
                    if nj == 1:
                        await message.channel.send(str(con1) + " 정부군의 부스트는 " + str(boost1) + "배,\n\n" + str(con2) + " 반란군의 부스트는 " + str(boost2) + "배 입니다.")
                        if conr1 == conr2:
                            await message.channel.send("```평지에서 " + str(con1) + " 내전은 비겼습니다.\n\n" + str(con1) + "의 전투력은 " + str(conr1) + ".\n\n" + str(con2) + "의 전투력은 " + str(conr2) + " 입니다.```")
                        elif conr1 > conr2:
                            result = conr1-conr2
                            await message.channel.send("```평지에서 " + str(con1) + " 내전은 " + str(con1) + "정부군의 승리로 끝이 났습니다.\n\n" + str(con1) + "정부군의 전투력은 " + str(conr1) + ".\n\n" + str(con2) + "반란군의 전투력은 " + str(conr2) + "이였으며\n\n두 세력간 전투력 격차는 " + str(result) + "입니다.\n\n\n==== 전략/전술 부스트 ====\n" + str(con1) + "정부군의 전략/전술 부스트는 " + str(boost1) + "배, \n" + str(con2) + " 반란군의 전략/전술 부스트는 " + str(boost2) + "배입니다.```")
                        elif conr1 < conr2:
                            result = conr2-conr1
                            await message.channel.send("```평지에서 " + str(con1) + " 내전은 " + str(con1) + "반란군의 승리로 끝이 났습니다.\n\n" + str(con1) + "반란군의 전투력은 " + str(conr2) + ".\n\n" + str(con1) + "정부군의 전투력은 " + str(conr1) + "이였으며\n\n두 세력간 전투력 격차는 " + str(result) + "입니다.\n\n\n==== 전략/전술 부스트 ====\n" + str(con1) + "반란군의 전략/전술 부스트는 " + str(boost2) + "배, \n" + str(con1) + " 정부군의 전략/전술 부스트는 " + str(boost1) + "배입니다.```")
                    elif nj == 0:
                        await message.channel.send(str(con1) + "의 부스트는 " + str(boost1) + "배,\n\n" + str(con2) + "의 부스트는 " + str(boost2) + "배 입니다.")
                        if conr1 == conr2:
                            await message.channel.send("```평지에서 " + str(con1) + ", " + str(con2) + "간 전쟁은 비겼습니다.\n\n" + str(con1) + "의 전투력은 " + str(conr1) + ".\n\n" + str(con2) + "의 전투력은 " + str(conr2) + " 입니다.```")
                        elif conr1 > conr2:
                            result = conr1-conr2
                            await message.channel.send("```평지에서 " + str(con1) + ", " + str(con2) + "간 전쟁은 " + str(con1) + "의 승리로 끝이 났습니다.\n\n" + str(con1) + "의 전투력은 " + str(conr1) + ".\n\n" + str(con2) + "의 전투력은 " + str(conr2) + "이였으며\n\n두 나라간 전투력 격차는 " + str(result) + "입니다.\n\n\n==== 전략/전술 부스트 ====\n" + str(con1) + "의 전략/전술 부스트는 " + str(boost1) + "배, \n" + str(con2) + "의 전략/전술 부스트는 " + str(boost2) + "배입니다.```")
                        elif conr1 < conr2:
                            result = conr2-conr1
                    ...
                    conr1 = con11*0.1 + con12*2 + con13*4 + con14*0.1 + con15*0.5 * boost1
                    conr2 = con21*0.1 + con22*2 + con23*4 + con24*0.1 + con25*0.5 * boost2
                    if nj ==1:
                        await message.channel.send(str(con1) + " 정부군의 부스트는 " + str(boost1) + "배,\n\n" + str(con2) + " 반란군의 부스트는 " + str(boost2) + "배 입니다.")
                        if conr1 == conr2:
